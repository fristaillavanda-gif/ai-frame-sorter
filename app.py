...
app = Flask(__name__)
app.secret_key = "super_secret_key_987654321"
    ...
    <script>
        ...
        analyzeBtn.addEventListener('click', async () => {
            ...
            if (!prompt || !apiKey || uploadedFiles.length === 0) {
                alert("Заполните все поля и загрузите картинки");
                return;
            }
        ...
        function simulateProgress(total) {
            ...
            const interval = setInterval(() => {
                ...
                if (p < 40) status.textContent = "Анализирую изображения...";
                else if (p < 75) status.textContent = "Сопоставляю с промптом...";
                else status.textContent = "Расставляю кадры...";
...
def analyze():
    ...
    try:
        ...
        # Сохраняем файлы для скачивания
...
def get_image_description(image_path, model):
    try:
        ...
        response = model.generate_content([
            "Опиши максимально точно и кратко, что происходит на картинке. 1-2 предложения.",
            img
        ])
